...
gpio_num_t gpio_03 = GPIO_NUM_14; //PINO D14 - Buzzer
...
void playTone(int frequency, int duration) {
    if (frequency == REST) {
        ...
        ledc_set_duty(LEDC_HIGH_SPEED_MODE, LEDC_CHANNEL_5, 512);
        ledc_update_duty(LEDC_HIGH_SPEED_MODE, LEDC_CHANNEL_5);

        vTaskDelay(duration / portTICK_PERIOD_MS);

        ledc_stop(LEDC_HIGH_SPEED_MODE, LEDC_CHANNEL_5, 0);
    }
}
...
void playMelody_mario(void *pvParameters) {
    ...
    for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
        ...
        playTone(melody[thisNote], noteDuration * 0.9);
        ESP_LOGI("melody", "duration %d",noteDuration);
        vTaskDelay(noteDuration / portTICK_PERIOD_MS);
    }
    vTaskDelete(NULL); // Delete the task when done
}
...
int execute_command(const char* command, int value) {
   ...
   ESP_LOGI(TAG, "Comando dentro execute: %s", command);
    ...
    else if (strcmp(command, "dur") == 0 || strcmp(command, "\"dur") == 0) { // define duração nota
    duracao_nota = value;
 
    }
    else if (strcmp(command, "buzzerm") == 0 || strcmp(command, "\"buzzerm") == 0) {
    buzzer_melody(value);
